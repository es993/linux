...
static int ad5758_slew_rate_set(struct ad5758_state *st,
				unsigned int sr_clk_index,
				unsigned int sr_step_index)
{
	unsigned int mode;
	unsigned long int mask;
	int ret;
...
{
	unsigned int sr_clk_index, sr_step_index;
	int i, sr_step_index_tmp;
	...
	for (i = 0; i < ARRAY_SIZE(ad5758_sr_clk); i++) {
		...
		if (diff_new < diff_old) {
			diff_old = diff_new;
			sr_clk_index = i;
			sr_step_index = sr_step_index_tmp;
		}
	}

	return ad5758_slew_rate_set(st, sr_clk_index, sr_step_index);
}
